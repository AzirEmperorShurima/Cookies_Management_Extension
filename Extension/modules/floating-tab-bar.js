...
(function () {
    ...
    *, *::before, *::after {
    ...
    // ==========================================
    // EXTENSION CONTEXT & SAFE API WRAPPERS
    // ==========================================
    ...
    function safeSendMessage(message, callback) {
        ...
        try {
            ...
            // Extension context invalidated, ignore gracefully
        }
    }
    ...
    function bindShadowEvents() {
        ...
        btnFooterDuplicates.addEventListener('click', (e) => {
            e.stopPropagation();
            safeSendMessage({ type: 'CLOSE_DUPLICATE_TABS' }, () => fetchTabsAndRender());
        });

        btnFooterSleep.addEventListener('click', (e) => {
            e.stopPropagation();
            safeSendMessage({ type: 'HIBERNATE_INACTIVE_TABS' }, () => fetchTabsAndRender());
        });

        btnFooterNewTab.addEventListener('click', (e) => {
            e.stopPropagation();
            safeSendMessage({ type: 'CREATE_NEW_TAB' });
        });
    ...
    async function loadAndRenderSavedSessions() {
        ...
        cachedSavedSessions.forEach((session, index) => {
            ...
            card.querySelector('.restore').addEventListener('click', (e) => {
                e.stopPropagation();
                safeSendMessage({ type: 'RESTORE_SESSION_TABS', tabs: session.tabs, inNewWindow: false });
                toggleSessionView();
            });
            ...
            card.querySelector('.new-win').addEventListener('click', (e) => {
                e.stopPropagation();
                safeSendMessage({ type: 'RESTORE_SESSION_TABS', tabs: session.tabs, inNewWindow: true });
                toggleSessionView();
            });
            ...
            card.querySelector('.delete').addEventListener('click', async (e) => {
                ...
                await safeSetStorage({ appSettings });
                loadAndRenderSavedSessions();
            });
    ...
    function executeSpotlightItem(item) {
        ...
        if (item.type === 'tab') {
            safeSendMessage({ type: 'FOCUS_TAB', tabId: item.id, windowId: item.windowId });
            return;
        }
        ...
        if (item.type === 'command') {
            switch (item.id) {
                case 'snap':
                    safeSendMessage({ type: 'ACTIVATE_PANIC' });
                    break;
                case 'clear':
                    safeSendMessage({ type: 'CLEAR_CURRENT_SITE_DATA', url: window.location.href });
                    break;
                case 'zap':
                    safeSendMessage({ type: 'ACTIVATE_ZAPPER' });
                    break;
                case 'pip':
                    safeSendMessage({ type: 'OPEN_PLAYER_FOR_TAB', url: window.location.href });
                    break;
                case 'tempmail':
                    safeSendMessage({ type: 'OPEN_POPUP_TAB', tab: 'tempMailSection' });
                    break;
                case 'sleep':
                    safeSendMessage({ type: 'HIBERNATE_INACTIVE_TABS' }, () => fetchTabsAndRender());
                    break;
    ...
    function fetchTabsAndRender(forceFullRender = false) {
        if (!isExtensionValid()) return;
        safeSendMessage({ type: 'GET_ALL_TABS_FOR_FLOATING_BAR' }, (response) => {
    ...
    function createPinnedPill(tab) {
        ...
        pill.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            safeSendMessage({ type: 'FOCUS_TAB', tabId: tab.id, windowId: tab.windowId });
        });
    ...
    function createGroupElement(groupInfo, groupTabs) {
        ...
        header.querySelector('button').addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            groupTabs.forEach(t => safeSendMessage({ type: 'CLOSE_TAB', tabId: t.id }));
        });
    ...
    function createTabItem(tab) {
        ...
        if (audioBtn) {
            audioBtn.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                safeSendMessage({ type: 'MUTE_TAB', tabId: tab.id, muted: !tab.mutedInfo?.muted });
            });
        }
        ...
        if (sleepBtn) {
            sleepBtn.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                safeSendMessage({ type: 'DISCARD_TAB', tabId: tab.id });
            });
        }
