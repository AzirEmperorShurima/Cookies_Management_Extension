...
import { debounce, createElement, ASSETS, atomicUpdateStorage } from './utils.js';
import { CookieSearchIndex } from './cookie-search.js';
export { CookieSearchIndex };
...
export async function saveSnapshot(domain, profileName) {
            ...
            const results = await chrome.scripting.executeScript({
                ...
                func: () => {
                    ...
                    try {
                        ...
                        extractionError = e.message || 'Storage Access Denied';
                    }
                    return { local, session, extractionError };
                }
            });
            if (results && results[0] && results[0].result) {
                localStorageData = results[0].result.local || {};
                sessionStorageData = results[0].result.session || {};
                if (results[0].result.extractionError) {
                    console.warn('[Snapshot] Storage extraction issue:', results[0].result.extractionError);
                }
            }
        }
    } catch (e) {
        console.warn(`[Snapshot] Could not extract web storage:`, e.message);
    }
    ...
    await atomicUpdateStorage('cookieSnapshots', (currentSnapshots = {}) => {
        const allSnapshots = { ...currentSnapshots };
...
export async function deleteSnapshot(domain, snapshotId) {
    ...
    await atomicUpdateStorage('cookieSnapshots', (currentSnapshots = {}) => {
        const allSnapshots = { ...currentSnapshots };
        if (!allSnapshots[cleanDomain]) return allSnapshots;
...
// ============ COOKIE EXPORTERS (JSON, NETSCAPE FORMAT) ============
export function convertCookiesToNetscape(cookies) {
    let output = '# Netscape HTTP Cookie File\n# Generated by Thanus Privacy Gauntlet\n\n';
    cookies.forEach(c => {
        ...
        const name = c.name || '';
        const value = c.value || '';
        output += `${domain}\t${includeSubdomains}\t${path}\t${isSecure}\t${expires}\t${name}\t${value}\n`;
    });
    return output;
}

const cookieSearchEngine = new CookieSearchIndex();
...
export async function loadCookies(filter = '', forceRefresh = false) {
    ...
    if (forceRefresh || cachedCookies.length === 0) {
        ...
        cookieSearchEngine.rebuild(cachedCookies);
    }
    ...
    // Security Tags Filter
    if (lowerFilter.startsWith('#secure') || lowerFilter.startsWith(':secure')) {
        matchedCookies = cachedCookies.filter(c => c.secure === true);
    } else if (lowerFilter.startsWith('#httponly') || lowerFilter.startsWith(':httponly')) {
        matchedCookies = cachedCookies.filter(c => c.httpOnly === true);
    } else if (lowerFilter.startsWith('#insecure') || lowerFilter.startsWith(':insecure')) {
        matchedCookies = cachedCookies.filter(c => c.secure === false);
    } else if (lowerFilter.startsWith('#session') || lowerFilter.startsWith(':session')) {
        matchedCookies = cachedCookies.filter(c => !c.expirationDate);
    } else if (lowerFilter.startsWith('#chips') || lowerFilter.startsWith(':chips')) {
        matchedCookies = cachedCookies.filter(c => Boolean(c.partitionKey || c.partitioned));
    } else if (lowerFilter) {
        // Fast Trie Search
        matchedCookies = cookieSearchEngine.search(lowerFilter);
    } else {
        matchedCookies = cachedCookies;
    }
