...
/**
 * Upload to Self-Hosted VPS Backend
 */
export async function uploadToSelfHostBackend(serverUrl, apiToken, password) {
    const { uploadToSelfHost } = await import('./selfhost.js');
    const data = await _gatherSyncData();
    const result = await uploadToSelfHost(serverUrl, apiToken, password, data);
    chrome.storage.session?.set({ pcm_sync_session_pass: password }).catch(() => {});
    return result;
}

/**
 * Restore from Self-Hosted VPS Backend
 */
export async function restoreFromSelfHostBackend(serverUrl, apiToken, password) {
    const { restoreFromSelfHost } = await import('./selfhost.js');
    const result = await restoreFromSelfHost(serverUrl, apiToken, password);
    await _applySyncData(result.data);
    chrome.storage.session?.set({ pcm_sync_session_pass: password }).catch(() => {});
    return result;
}
...
export async function syncToCloud() {
    ...
    try {
        ...
        if (settings.syncBackend === 'selfhost' && settings.selfHostUrl && settings.selfHostToken) {
            await uploadToSelfHostBackend(settings.selfHostUrl, settings.selfHostToken, password);
        } else if (settings.syncBackend === 'drive') {
...
export async function syncFromCloud(keys) {
    ...
    let data;
    if (settings.syncBackend === 'selfhost' && settings.selfHostUrl && settings.selfHostToken) {
        const result = await restoreFromSelfHostBackend(settings.selfHostUrl, settings.selfHostToken, password);
        data = result.data;
    } else if (settings.syncBackend === 'drive') {
...
export function initSyncUI() {
    ...
    const selfHostBkBtn = document.getElementById('syncBackendSelfHost');
    const driveSection  = document.getElementById('driveAuthSection');
    const selfHostSection = document.getElementById('selfHostConfigSection');
    const selfHostUrlInput = document.getElementById('selfHostUrlInput');
    const selfHostTokenInput = document.getElementById('selfHostTokenInput');
    const toggleSelfHostTokenEye = document.getElementById('toggleSelfHostTokenEye');
    const pingSelfHostBtn = document.getElementById('pingSelfHostBtn');
    const saveSelfHostConfigBtn = document.getElementById('saveSelfHostConfigBtn');
    const selfHostPingStatus = document.getElementById('selfHostPingStatus');
    const selfHostRelayToggle = document.getElementById('selfHostRelayToggle');
    const selfHostMailToggle = document.getElementById('selfHostMailToggle');
    ...
    // ── Load Self-Host saved inputs ──
    if (selfHostUrlInput && settings.selfHostUrl) selfHostUrlInput.value = settings.selfHostUrl;
    if (selfHostTokenInput && settings.selfHostToken) selfHostTokenInput.value = settings.selfHostToken;
    if (selfHostRelayToggle) selfHostRelayToggle.checked = settings.selfHostRelayEnabled !== false;
    if (selfHostMailToggle) selfHostMailToggle.checked = settings.selfHostMailEnabled !== false;
    ...
    async function refreshSyncInfo() {
        ...
        try {
            ...
            if (backend === 'selfhost') {
                if (settings.selfHostUrl && settings.selfHostToken) {
                    const { getSelfHostBackupHistory } = await import('./selfhost.js');
                    const history = await getSelfHostBackupHistory(settings.selfHostUrl, settings.selfHostToken);
                    lastSyncInfo.textContent = history?.length
                        ? `Last backup: ${_formatRelativeTime(history[0].timestamp)} (${history.length} versions)`
                        : 'No VPS backup found';
                } else {
                    lastSyncInfo.textContent = 'VPS unconfigured';
                }
            } else if (backend === 'drive') {
                const meta = await getDriveLastSyncMeta();
                lastSyncInfo.textContent = meta
                    ? `Last backup: ${_formatRelativeTime(meta.timestamp)}${_formatSize(meta.totalSize || meta.size)}`
                    : 'No backup found';
            } else {
                const meta = await getChromeLastSyncMeta();
                lastSyncInfo.textContent = meta
                    ? `Last backup: ${_formatRelativeTime(meta.timestamp)}${_formatSize(meta.totalSize || meta.size)}`
                    : 'No backup found';
            }
    ...
    if (syncEnabledToggle) {
        ...
        syncEnabledToggle.addEventListener('change', e => {
            ...
            saveSettings();
            notify(`Encrypted Cloud Sync ${settings.syncEnabled ? 'đã bật' : 'đã tắt'}`, 'success');
        });
    }
    ...
    function updateBackendUI() {
        ...
        const isChrome = !isDrive && !isSelfHost;

        if (chromeBkBtn) {
            chromeBkBtn.style.background = isChrome ? 'rgba(162,155,254,0.25)' : 'transparent';
            chromeBkBtn.style.color      = isChrome ? '#a29bfe' : 'var(--text-muted, #888)';
            chromeBkBtn.style.border     = isChrome ? '1px solid rgba(162,155,254,0.6)' : '1px solid rgba(255,255,255,0.1)';
        }
        if (driveBkBtn) {
            ...
            driveBkBtn.style.border     = isDrive ? '1px solid rgba(52,168,83,0.5)' : '1px solid rgba(255,255,255,0.1)';
        }
        if (selfHostBkBtn) {
            selfHostBkBtn.style.background = isSelfHost ? 'rgba(108,92,231,0.3)' : 'transparent';
            selfHostBkBtn.style.color      = isSelfHost ? '#a29bfe' : 'var(--text-muted, #888)';
            selfHostBkBtn.style.border     = isSelfHost ? '1px solid rgba(108,92,231,0.6)' : '1px solid rgba(255,255,255,0.1)';
        }

        if (driveSection) driveSection.style.display = isDrive ? 'block' : 'none';
        if (selfHostSection) selfHostSection.style.display = isSelfHost ? 'block' : 'none';
        refreshSyncInfo();
    }
    ...
    selfHostBkBtn?.addEventListener('click', () => {
        settings.syncBackend = 'selfhost';
        saveSettings();
        updateBackendUI();
    });

    // ── Self-Host Ping & Save ──
    pingSelfHostBtn?.addEventListener('click', async () => {
        const url = selfHostUrlInput?.value.trim();
        const token = selfHostTokenInput?.value.trim();
        if (!url) {
            notify('Please enter VPS Server URL first', 'error');
            return;
        }

        if (selfHostPingStatus) {
            selfHostPingStatus.textContent = 'Pinging...';
            selfHostPingStatus.style.color = '#fdcb6e';
        }

        try {
            const { pingSelfHost } = await import('./selfhost.js');
            const res = await pingSelfHost(url, token);
            if (selfHostPingStatus) {
                selfHostPingStatus.textContent = `Online (${res.latencyMs}ms)${res.authenticated ? ' 🔒' : ' ⚠️'}`;
                selfHostPingStatus.style.color = res.authenticated ? '#00b894' : '#fdcb6e';
            }
            notify(`VPS Online (${res.latencyMs}ms) - ${res.authenticated ? 'Authenticated' : 'Unauthenticated'}`, 'success');
        } catch (err) {
            if (selfHostPingStatus) {
                selfHostPingStatus.textContent = 'Offline / Error';
                selfHostPingStatus.style.color = '#ff7675';
            }
            notify(`Cannot connect to VPS: ${err.message}`, 'error');
        }
    });

    saveSelfHostConfigBtn?.addEventListener('click', () => {
        settings.selfHostUrl = selfHostUrlInput?.value.trim() || '';
        settings.selfHostToken = selfHostTokenInput?.value.trim() || '';
        if (selfHostRelayToggle) settings.selfHostRelayEnabled = selfHostRelayToggle.checked;
        if (selfHostMailToggle) settings.selfHostMailEnabled = selfHostMailToggle.checked;
        saveSettings();
        notify('Self-Host VPS configuration saved!', 'success');
        refreshSyncInfo();
    });

    toggleSelfHostTokenEye?.addEventListener('click', () => {
        if (!selfHostTokenInput) return;
        const isHidden = selfHostTokenInput.type === 'password';
        selfHostTokenInput.type = isHidden ? 'text' : 'password';
        if (toggleSelfHostTokenEye) toggleSelfHostTokenEye.textContent = isHidden ? '🙈' : '👁️';
    });
