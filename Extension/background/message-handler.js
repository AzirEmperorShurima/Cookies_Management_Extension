...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.type === 'ZAP_ELEMENT') {
        ...
        atomicUpdateStorageBackground('userZappedCssRules', (currentRules = {}) => {
            const rules = { ...currentRules };
            if (!rules[domain]) rules[domain] = [];
            if (!rules[domain].includes(selector)) {
                rules[domain] = [...rules[domain], selector];
            }
            return rules;
        }).then(() => {
...
async function _fetchEasyListInBackground() {
    ...
    try {
        ...
        const easyListCssRuleSets = {}; // domain -> Set of selectors

        const BATCH_SIZE = 5000;
        for (let i = 0; i < lines.length; i++) {
            // Yield event loop every BATCH_SIZE lines to prevent service worker watchdog freeze
                ...
                if (selector) {
                    if (domainsPart) {
                        ...
                        for (let d = 0; d < domains.length; d++) {
                            const domain = domains[d].trim();
                            if (!domain || domain.startsWith('~')) continue; // Bỏ qua domain phủ định
                            if (!easyListCssRuleSets[domain]) easyListCssRuleSets[domain] = new Set();
                            easyListCssRuleSets[domain].add(selector);
                        }
                    } else {
                        if (!easyListCssRuleSets['global']) easyListCssRuleSets['global'] = new Set();
                        easyListCssRuleSets['global'].add(selector);
                    }
                }
            }
        }
        ...
        for (const [dom, set] of Object.entries(easyListCssRuleSets)) {
            easyListCssRules[dom] = Array.from(set);
        }
                    ...
                    if (selector) {
                        if (domainsPart) {
                            domainsPart.split(',').forEach(d => {
                                ...
                                if (!customCssRules[domain].includes(selector)) {
                                    customCssRules[domain].push(selector);
                                }
                            });
                        } else {
                            customCssRules['global'] = customCssRules['global'] || [];
                            if (!customCssRules['global'].includes(selector)) {
        ...
        const compiledCssRules = (isAdblockOn && isEasylistOn) 
            ? Object.fromEntries(Object.entries(easyListCssRules).map(([k, v]) => [k, [...v]])) 
            : {};
        if (isAdblockOn) {
