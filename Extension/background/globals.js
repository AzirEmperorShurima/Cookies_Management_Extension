...
// ─── Atomic Storage Helper for Service Worker ────────────────────────────────
const _bgStorageMutationQueues = new Map();

function atomicUpdateStorageBackground(key, updaterFn, area = 'local') {
    const queueKey = `${area}:${key}`;
    let queue = _bgStorageMutationQueues.get(queueKey) || Promise.resolve();
    ...
    const nextPromise = queue.then(async () => {
        try {
            ...
            console.error(`[Background Globals] Error in atomicUpdateStorageBackground for ${queueKey}:`, err);
            return null;
        }
    }).catch(err => {
        console.error(`[Background Globals] Uncaught queue error for ${queueKey}:`, err);
        return null;
    });

    _bgStorageMutationQueues.set(queueKey, nextPromise);
    return nextPromise;
}

function updateAppSettingsBackground(updaterFn) {
    return atomicUpdateStorageBackground('appSettings', updaterFn, 'local');
}
