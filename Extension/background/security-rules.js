...
async function updateSecurityRules() {
    ...
    // 1.2 Privacy Player - Maximum Identity Isolation & Anti-Leak Shield (tabIds: [-1])
    // Strips ambient browser cookies (Google accounts, tracking cookies), auth headers, Chrome telemetry (X-Client-Data)
    // and enforces Global Privacy Control (Sec-GPC) & Do Not Track (DNT)
    if (settings.playerIsolatedIdentity !== false) {
        sessionRulesToAdd.push({
            id: 2004,
            priority: 4,
            action: {
                ...
                requestHeaders: [
                    ...
                    { header: 'Proxy-Authorization', operation: 'remove' },
...
// Backup alarm: flush every 1 min (compliant with MV3 minimum alarm interval)
chrome.alarms.create('flushDailyStats', { periodInMinutes: 1 });

// Ensure stats are flushed immediately when Service Worker is being suspended by the browser
if (chrome.runtime && chrome.runtime.onSuspend) {
    chrome.runtime.onSuspend.addListener(flushDailyStats);
}
