...
export function detectAndApplyDisplayMode() {
    try {
        const urlParams = new URLSearchParams(window.location.search);
        const mode = urlParams.get('mode');
        // Explicit full tab mode only when requested via parameter or index.html
        const isTab = mode === 'tab' || window.location.pathname.includes('index.html');
        const isPopup = (chrome.extension && typeof chrome.extension.getViews === 'function') 
                        ? chrome.extension.getViews({ type: 'popup' }).includes(window) 
                        : false;
        // Side panel mode is when mode=sidepanel OR when NOT a standalone tab and NOT popup dropdown (e.g. docked side panel)
        const isSidePanel = mode === 'sidepanel' || (!isTab && !isPopup && window.innerHeight > 600);

        if (isTab) {
            document.documentElement.classList.add('full-tab');
            document.body.classList.add('full-tab');
            document.documentElement.classList.remove('side-panel');
            document.body.classList.remove('side-panel');
        } else if (isSidePanel) {
            document.documentElement.classList.add('side-panel');
            document.body.classList.add('side-panel');
            document.documentElement.classList.remove('full-tab');
            document.body.classList.remove('full-tab');
        } else {
            document.documentElement.classList.remove('full-tab', 'side-panel');
            document.body.classList.remove('full-tab', 'side-panel');
        }
    } catch (e) { }
}
...
export function updateUILanguage() {
    ...
    // Detect panel / full tab display mode
    detectAndApplyDisplayMode();

    // Preserve scroll position of settings container
    const settingsContainer = elements.settingsContainer || document.getElementById('settingsContainer');
    const prevScrollTop = settingsContainer ? settingsContainer.scrollTop : null;
    ...
    document.querySelectorAll('[data-i18n]').forEach(el => {
        ...
        if (dict[key]) {
            ...
            // Fast-check if translation text hasn't changed to avoid wiping DOM nodes
            const currentNonBadgeText = Array.from(el.childNodes)
                .filter(n => n.nodeType === Node.TEXT_NODE)
                .map(n => n.textContent.trim())
                .join(' ');
            if (currentNonBadgeText === translatedText.trim()) return;
    ...
    if (prevScrollTop !== null && settingsContainer) {
        settingsContainer.scrollTop = prevScrollTop;
    }

    // Re-assert select values to ensure no dropdown becomes blank after translation
    if (elements.playerLinkBehavior) elements.playerLinkBehavior.value = settings.linkClickBehavior || 'inside';
    if (elements.playerLinkFilter) elements.playerLinkFilter.value = settings.appliedLinkType || 'all';
    if (elements.protectionLevelSelect) elements.protectionLevelSelect.value = settings.protectionLevel || 'standard';
    if (elements.searchEngineSelect) elements.searchEngineSelect.value = settings.searchEngine || 'google';
...
export async function toggleSection(section) {
    ...
    if (section === 'adblock') {
        ...
        if (extensionManager) extensionManager.classList.add('active');
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.querySelectorAll('.info-icon').forEach(icon => {
        ...
        icon.addEventListener('mouseenter', (e) => {
            ...
            // Calculate vertical position relative to fixed viewport
    ...
    try {
        ...
        console.warn('[Popup] doNotTrackEnabled query skipped/unavailable:', err);
    }
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
        if (area === 'local' && changes.appSettings) {
            ...
            const newLang = changes.appSettings.newValue?.language;
            settings = { ...settings, ...changes.appSettings.newValue };
            applySettings(oldLang !== newLang);
        }
    });

    // Dynamic resize handler for responsive fluid layout across popup/panel/tab modes
    ...
    // Horizontal scrolling on mousewheel hover and drag for navigation bar
    const mainControls = document.querySelector('.mainControls');
    if (mainControls) {
        // Prevent default HTML5 drag ghosting on images/links inside mainControls
        mainControls.querySelectorAll('img, a').forEach(el => {
            el.setAttribute('draggable', 'false');
        });

        mainControls.addEventListener('wheel', (e) => {
            const hasVertical = Math.abs(e.deltaY) > 0;
            const hasHorizontal = Math.abs(e.deltaX) > 0;
            if (hasVertical || hasHorizontal) {
                e.preventDefault();
                const rawDelta = hasHorizontal && Math.abs(e.deltaX) >= Math.abs(e.deltaY) ? e.deltaX : e.deltaY;
                const multiplier = e.deltaMode === 1 ? 33 : (e.deltaMode === 2 ? 300 : 1);
                mainControls.scrollLeft += rawDelta * multiplier;
            }
        }, { passive: false });
        ...
        mainControls.addEventListener('mousedown', (e) => {
            if (e.button !== 0) return;
            isDown = true;
            hasDragged = false;
            startX = e.pageX - mainControls.offsetLeft;
            scrollStart = mainControls.scrollLeft;
        });

        window.addEventListener('mouseup', () => {
            if (isDown) {
                isDown = false;
                mainControls.classList.remove('active-drag');
                setTimeout(() => {
                    hasDragged = false;
                }, 50);
            }
        ...
        mainControls.addEventListener('mousemove', (e) => {
            if (!isDown) return;
            const x = e.pageX - mainControls.offsetLeft;
            const walk = (x - startX);
            if (Math.abs(walk) > 4) {
                hasDragged = true;
                mainControls.classList.add('active-drag');
                e.preventDefault();
                mainControls.scrollLeft = scrollStart - (walk * 1.3);
            }
        });
