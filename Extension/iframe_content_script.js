...
(function () {
    ...
    if (window.self !== window.top && isExtensionFrame) {
        ...
        const AD_HOST_PATTERNS = [
            ...
            'linkroyal', 'linkvertise', 'ouo.io', 'adf.ly', 'adbtc', 'shortlink', 'miss.ai'
        ];
        ...
        const SUSPICIOUS_SERVERLESS_DOMAINS = [
            '.workers.dev', '.pages.dev', '.r2.dev', '.b-cdn.net'
        ];
        ...
        function isKnownAdUrl(targetUrl) {
            ...
            try {
                ...
                // 1. Direct host pattern match
                if (AD_HOST_PATTERNS.some(d => host.includes(d))) return true;

                // 2. Gambling keyword in host or pathname
                if (GAMBLING_KEYWORDS.some(kw => host.includes(kw) || parsed.pathname.toLowerCase().includes(kw))) {
                    return true;
                }

                // 3. Shortlink serverless workers (workers.dev/pages.dev) cross-origin
                if (SUSPICIOUS_SERVERLESS_DOMAINS.some(sd => host.endsWith(sd))) {
                    if (host.includes('link') || host.includes('ad') || host.includes('short') || host.includes('click') || host.includes('tracker') || GAMBLING_KEYWORDS.some(kw => fullUrl.includes(kw))) {
                        return true;
                    }
                    return true;
                }

                // 4. Common suspicious ad redirect query params
                if (parsed.searchParams.has('redirect') || parsed.searchParams.has('target') || parsed.searchParams.has('dest') || parsed.searchParams.has('out')) {
                    const redirectTarget = parsed.searchParams.get('redirect') || parsed.searchParams.get('target') || parsed.searchParams.get('dest') || parsed.searchParams.get('out');
                    if (redirectTarget && GAMBLING_KEYWORDS.some(kw => redirectTarget.toLowerCase().includes(kw))) {
                        return true;
                    }
                }
        ...
        function isOverlayElement(el) {
            ...
            try {
                ...
                const isTransparent = parseFloat(style.opacity) < 0.15 || style.visibility === 'hidden' || style.backgroundColor === 'rgba(0, 0, 0, 0)' || style.backgroundColor === 'transparent';
                const isPositioned = style.position === 'absolute' || style.position === 'fixed';
                const zIndex = parseInt(style.zIndex, 10);
                const hasHighZIndex = !isNaN(zIndex) && zIndex >= 10;
                
                if (isPositioned && hasHighZIndex) {
                    const rect = el.getBoundingClientRect();
                    if (rect.width > 200 && rect.height > 150) {
                        return true;
                    }
                }
                if (el.tagName === 'A' && isPositioned && hasHighZIndex) {
                    return true;
                }
            } catch (e) {}
            return false;
        ...
        function forwardClickToVideo(clickEvent) {
            try {
                // Find underlying video on page
                ...
                // Fallback: Dispatch click to element behind the blocked target
                if (clickEvent && typeof clickEvent.clientX === 'number' && typeof clickEvent.clientY === 'number') {
                    const elementsUnder = document.elementsFromPoint(clickEvent.clientX, clickEvent.clientY);
                    for (const el of elementsUnder) {
                        if (el !== clickEvent.target && !isOverlayElement(el)) {
                            if (el.tagName === 'VIDEO') {
                                if (el.paused) el.play().catch(() => {}); else el.pause();
                                return;
                            }
                            if (el.classList.contains('play') || el.getAttribute('aria-label')?.includes('Play') || el.tagName === 'BUTTON') {
                                el.click();
                                return;
                            }
                        }
        ...
        function autoDismissVideoAds() {
            ...
            const adOverlays = document.querySelectorAll('div[class*="ad-overlay"], div[id*="ad_overlay"], a[href*="tsyndicate"], a[href*="adsterra"], a[href*="workers.dev"], a[href*="linkroyal"]');
        ...
        document.addEventListener('click', (event) => {
            ...
            while (target && target !== document) {
                ...
                // 2. Chặn quảng cáo click-trap / overlay tức thì & chuyển tiếp sự kiện Play tới video
                if (isAd && settings.adblockEnabled) {
                    event.preventDefault();
                    event.stopPropagation();
                    console.log('[Privacy Player Shield] Blocked ad trap link click & forwarded to video:', finalUrl);
                    try {
                        if (target && target.parentNode) target.remove();
                    } catch (e) {}
                    forwardClickToVideo(event);
                    return;
                }
                ...
                // 4. Xử lý theo Hành Vi đã chọn với cơ chế bảo vệ chống cướp tab/trình phát
                if (behavior === 'block') {
                    event.preventDefault();
                    event.stopPropagation();
                    try {
                        if (isOverlayElement(target)) target.remove();
                    } catch (e) {}
                    forwardClickToVideo(event);
                    chrome.runtime.sendMessage({
                ...
                if (behavior === 'smart') {
                    ...
                    event.stopPropagation();
                    if (isAd) {
                        console.log('[Privacy Player Shield] Smart Mode suppressed ad redirect:', finalUrl);
                        try {
                            if (target && target.parentNode) target.remove();
                        } catch (e) {}
                        forwardClickToVideo(event);
                ...
                if (behavior === 'inside') {
                    if (requiresNewTab) {
                        event.preventDefault();
                        event.stopPropagation();
                        // Ngăn chặn chuyển hướng nội bộ iframe sang web cờ bạc / quảng cáo ngoài
                        if (isAd || isExternal) {
                            console.log('[Privacy Player Shield] Inside Mode blocked external/ad target:', finalUrl);
                            try {
                                if (target && target.parentNode) target.remove();
                            } catch (e) {}
                            forwardClickToVideo(event);
